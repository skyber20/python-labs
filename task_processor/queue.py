...
class TaskQueue:
    ...
    def high_priority(self, min_priority: int) -> Iterator[Task]:
        """
        Ленивый фильтр по приоритету
        
        :param min_priority: Минимальный приоритет (0-10)
        :return: Генератор задач с приоритетом >= min_priority
        """
        if not isinstance(min_priority, int) or min_priority < 0 or min_priority > 10:
            raise ConfigError("min_priority должно быть int в диапазоне 0-10")
        
        higher_priority = lambda t: t.priority >= min_priority
        return self.filter(higher_priority)
